...
public class Step05ClassTest extends PlainTestCase {
    ...
    // #1on1: $買えたときの方がifの真になってる方がしっくりくる (2026/09/24)
    // ぼくも if/elseのときに、できるだけifの方に本編を持ってくるように調整することある。
    // やるなら、if (quantity <= 0) { の方もそのように合わせてもいいのかも。
    // $if (quantity <= 0) {の処理はそれで一つのカテゴリという感覚だった
    // 一方で、early returnのように、前提条件を先にチェックして、最後ゆっくり本編みたいなデザインもある。
    //
    // jfluteの感覚:
    // その例外ケースが、単なる前提条件の破れなのか？わりかしれっきとしてサブケースなのか？
    // それによるかも。
    //
    // $ifを書いたらelseを書きたくなる。elseがない不安がまだある!?
    // 構造化プログラミングの話を思い出した。出口を複数作らない。
    //
    // ifのネストとかで前提条件の破れケースの把握がしづらくなるかも。
    // ゆえに、「ここに来たら前提条件は揃ってる」を重視される傾向にあるのかも。
    //
    // $聞いてると、業務メインケースのifの条件が複雑になりやすいのかも!?
    //
    // そう考えるとif文のデザインって意外に難しい。それなりに選択肢があって、常に考えてる。
    // どっちの書き方でもいいけど、この場面だとどっちが直感的かな？って。
    //
    // webサービスのDB使う現場だと、ロールバックがあるから、多少曖昧でも大丈夫だったり。
    // 普通のインスタンス変数とか使ったものだと、前提条件すべてを最初にチェックしないといけなかったり。
    ...
    // #1on1: いいね、その感覚正しいです。コメントの残しているの素晴らしい。 (2026/09/24)
